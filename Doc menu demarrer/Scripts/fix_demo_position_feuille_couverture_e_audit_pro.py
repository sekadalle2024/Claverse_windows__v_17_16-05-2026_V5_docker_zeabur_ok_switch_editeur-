...
PROBLEME :
  Dans le mode [Demo] de l'étape "Feuille couverture" (E-audit Pro, Phase de réalisation),
  la ligne `[Demo] = Activate` est placée AVANT la ligne `[Nb de lignes] = 25`.
...
SOLUTION :
  La ligne `[Demo] = Activate` doit être placée APRES la ligne `[Nb de lignes] = 25`,
...
def fix_demo_position(content: str) -> tuple[str, int]:
    """
    Déplace la ligne [Demo] = Activate de la position avant [Nb de lignes]
    vers la fin de la commande, après [Nb de lignes] = X.

    Stratégie :
      - Cherche TOUTES les occurrences où [Demo] = Activate apparaît avant [Nb de lignes]
      - Les remplace en déplaçant [Demo] = Activate après [Nb de lignes]

    Utilise re.sub avec DOTALL pour gérer les blocs multilignes.
    """

    # Pattern : [Demo] = Activate placé avant [Nb de lignes] = N (N peut être n'importe quel nombre)
    pattern = re.compile(
        r'\[Demo\] = Activate\n'                # ligne Demo (à déplacer)
        r'(\[Nb de lignes\] = \d+)',            # ligne Nb de lignes (capture le nombre)
        re.DOTALL
    )
...
def process_file(filepath: str) -> bool:
    ...
    # Vérification : s'assurer qu'il ne reste plus de [Demo] avant [Nb de lignes]
    remaining = len(re.findall(r'\[Demo\] = Activate\n\[Nb de lignes\]', new_content))
    if remaining > 0:
...
def print_summary(count: int):
    ...
    print("   2. Tester le menu Demarrer > E-audit Pro > Phase de realisation")
    print("   3. Selectionner 'Feuille couverture' et verifier le mode [Demo]")
...
if __name__ == '__main__':
    print("\n" + "=" * 70)
    print("CORRECTION POSITION [Demo] = Activate - E-audit Pro")
    print("=" * 70)
    print("   Section : TOUTES les etapes avec mode Demo")
