...
(function () {
  ...
  function getBackendBaseUrl() {
    ...
    if (BACKEND_ENV_MAP[origin]) return BACKEND_ENV_MAP[origin];
    if (typeof window.__VITE_BACKEND_URL__ !== "undefined" && window.__VITE_BACKEND_URL__)
      return window.__VITE_BACKEND_URL__;
    console.warn(`⚠️ [Config] Origine inconnue: ${origin}. Fallback → http://localhost:5000`);
  ...
  const CONFIG = {
    ...
    ETAT_FIN: {
      ...
      endpoint: `${getBackendBaseUrl()}/etats-financiers/process-excel`,
