...
Write-Host "  Push ClaraVerse V17 en Commits Multiples                      " -ForegroundColor Cyan
...
if ($status) {
    ...
    Write-Host "  $(($status | Measure-Object).Count) fichiers a traiter" -ForegroundColor Gray
} else {
...
# Vérifier le remote
Write-Host ""
Write-Host "3. Vérification du repository distant..." -ForegroundColor Yellow
$currentRemote = git remote get-url origin
if ($currentRemote -eq $repoUrl) {
    Write-Host "  ✅ Repository correct: $repoUrl" -ForegroundColor Green
} else {
    Write-Host "  ⚠️ Changement du repository distant..." -ForegroundColor Yellow
    git remote set-url origin $repoUrl
    Write-Host "  ✅ Repository mis a jour: $repoUrl" -ForegroundColor Green
}
...
if ($currentBranch -eq $branche) {
    ...
    Write-Host "  ℹ️ Le script utilisera la branche: $branche" -ForegroundColor Cyan
}
...
git add *.md *.txt "Doc_Github_Issue/" "Doc Koyeb deploy/" "Doc zeabur docker/" "Doc backend switch/" "deploiement-netlify/" "Doc ClaraAttachmentService traitement/"
